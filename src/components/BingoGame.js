...
const BingoGame = () => {
    ...
    const checkWinner = (board) => {
      ...
      for (let row = 0; row < 5; row++) {
        if (board[row].every((cell) => cell.color === 'red')) {
          const calledNumber = selectedNumber; // Get the called number when the board wins
  ...
  return (
    <div className="bingo-container">
      ...
      {gameStarted && selectedNumber && <h2>Selected Number: {selectedNumber}</h2>}
      ...
      {winner && <h2 className=''>{`${winner} Wins! Congrats!`}</h2>}
      {winner && <h2 className=''>{ ` with Score : ${score} `}  &#128516;&#127881;&#127882;</h2>}
      {gameOver && <h2 className=''>The game is over.</h2>}
    </div>
  );
};
